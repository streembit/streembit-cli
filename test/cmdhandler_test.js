...
import { CmdHandler } from "../apps/cmd/index.js";
...
import { EccKey as ecckey } from "../libs/crypto/index.js";
...
describe("CMD Handlers", function () {
  ...
  before(function (done) {
    config.init(
      config_json.transport.port,
      config_json.transport.host,
      true,
      false,
      () => {
        cmd = new CmdHandler();
  ...
  describe("Account commands handler", function () {
    let run_account_tests = true,
      pk = null,
      account = new Account();
    const sq3 = new sqlite3.Database(
      "db/sqlite/streembittest/streembittest.db",
    ...
    if (run_account_tests) {
      it("should validate input for account name", function () {
        const validchars =
          "abcdefghijklm_nopqrstuvwxyz-ABCDEFGHIJKLM-NOPQRSTUVWXYZ0_123456789";
        const name = Array(12)
          .join()
          .split(",")
          .map(() =>
  ...
  describe("User commands handler", function () {
    ...
    it("should have a schema for add/update prompt", function () {
      ...
      try {
        assert.hasAllKeys(usr.properties, [
          "username",
          "pk",
          "isadmin",
          "settings",
    ...
    it("should validate JSON string", function () {
      ...
      assert.isTrue(
        users.validateJSON(
          '[{"fst":1,"snd":2,"thd":3},{"thd":3,"snd":2,"fst":1}]'
        )
      );
