...
import * as bcdb from './bcdatabase/index.js';


import { BcKey } from './bcjs/index.js';
import { BcNode } from './bcnode/index.js'
import { NodeInfo } from './bcnode/nodeinfo.js';
import { template as ForgeTxn } from './bcjs/txns/forge/index.js';
import { Block } from './bcjs/block.js';
import { Account } from '../../libs/account/index.js';
import { template as GenesisTxnHandler } from './bcjs/txns/genesis/index.js';
import { template as ForgeTxnHandler } from './bcjs/txns/forge/index.js';
...
function createBlock() {
    ...
    bcdb.getLastBlockno(
        (err, lastblockno) => {
            ...
            let blockdata = BcNode.GetBlockFileData(lastblockno);
            let params = {
                starttime: Date.now(),
...
function CreateGenesis(privkhex, callback) {
    try {
        ...
        let data = BcNode.GetBlockFileData(0);
        console.log(data.blockid == blockid);
        let importtx = BcNode.getTxnFromHex(data.transactions[0].txnhex);
        console.log(txhash == importtx.getHashHex());
        let importblock = BcNode.getBlockFromHex(data.blockhex);
...
function ForgeTransaction(blockNo, privkeyHex, forgeAddress, amount, callback) {
    try {
        ...
        let blockdata = BcNode.GetBlockFileData(blockNo - 1);
        let params = {
            starttime: Date.now(),
            prevblock: blockdata.blockid,
            bckey: bckey
