...
var btcjs = require('../bcjs')

var NETWORKS = btcjs.networks
...
Wallet.fromJSON = function (json, network) {
  function toChain (cjson) {
    var node = btcjs.HDNode.fromBase58(cjson.node, network)
...
Wallet.fromSeedBuffer = function (seed, network) {
  ...
  var m = btcjs.HDNode.fromSeedBuffer(seed, network)
...
Wallet.prototype.buildTransaction = function (inputs, outputs, feeMax, external, internal, nLockTime) {
  ...
  outputs = outputs.map(function (output) {
    ...
    if (!script && output.address) {
      script = btcjs.address.toOutputScript(output.address, network)
    }

    if (!script) {
      script = btcjs.address.toOutputScript(chain.get(), network)
      chain.next()
    }
  ...
  // build transaction
  var txb = new btcjs.TransactionBuilder(network)
