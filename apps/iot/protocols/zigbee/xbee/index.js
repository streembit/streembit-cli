...
class XbeeHandler {
  ...
  swapEUI64toLittleEndian(eui64) {
    if (!eui64 || eui64.length != 16) {
      throw new Error(
        "Invalid EUI64 data. EUI64 must be an 16 charecter long string"
      );
    }
  ...
  swapEUI64BuffertoBigEndian(eui64) {
    if (!eui64 || eui64.length != 8 || !Buffer.isBuffer(eui64)) {
      throw new Error(
        "Invalid EUI64 buffer, eui64 param must be a 8 byte lenght buffer"
      );
    }
  ...
  handle_cluster_8004(frame) {
    ...
    if (status != 0) {
      return logger.error(
        "Simple Descriptor Request status: error for device " + frame.remote64
      );
    }
    ...
    setTimeout(() => {
      ...
      this.simple_basciccluster_request(
        frame.remote64,
        frame.remote16,
        endpoint,
        bcrdata
  ...
  handle_cluster_0b04_01(frame, reader) {
    ...
    function read_next_attribute() {
      try {
        ...
        if (property_name) {
          logger.debug(
            `property: ${property_name} datatype:${datatype}, value: ${value}`
          );
          return {
  ...
  handle_cluster_0b04(frame) {
    ...
    if (zcl_command == 0x0a) {
      // ZCL 0x0a Report attributes 7.11
      //logger.debug(util.inspect(frame));
      this.handle_cluster_0b04_0a(frame, reader);
    } else if (zcl_command == 0x01) {
      // ZCL 0x01 Read attributes response 7.2
  ...
  handle_cluster_0402_01(frame, reader) {
    ...
    var status = reader.nextUInt8();
    if (status != 0) {
      return this.dispatch_error_event(
        frame.remote64,
        "cluster 0402 invalid status returned"
      );
    }
    ...
    if (datatype != 0x29) {
      return this.dispatch_error_event(
        frame.remote64,
        "cluster 0402 invalid data type returned"
      );
    }
  ...
  handle_cluster_0402(frame) {
    ...
    var zcl_command = reader.nextUInt8();
    logger.debug(
      "cluster 0402 zcl_command: %s",
      sprintf("0x%02x", zcl_command)
    );
    ...
    } else if (zcl_command == 0x07) {
      // ZCL 0x07 Configure reporting response 7.8
      logger.debug(
        `handle_cluster_0402 zcl_command == 0x07 ${util.inspect(frame)}`
      );
      var status = reader.nextUInt8();
      if (status == 0x00) {
        this.dispatch_datarcv_event({
  ...
  handle_cluster_0405_01(frame, reader) {
    ...
    if (status != 0) {
      return this.dispatch_error_event(
        frame.remote64,
        "cluster 0405 invalid status returned"
      );
    }
    ...
    if (datatype != 0x21) {
      return this.dispatch_error_event(
        frame.remote64,
        "cluster 0405 invalid data type returned"
      );
    }
  ...
  handle_cluster_0405(frame) {
    ...
    if (zcl_command == 0x0a) {
      ...
      // ZCL 0x07 Configure reporting response 7.8
      logger.debug(
        `handle_cluster_0405 zcl_command == 0x07 ${util.inspect(frame)}`
      );
      var status = reader.nextUInt8();
      if (status == 0x00) {
        this.dispatch_datarcv_event({
  ...
  handle_cluster_8021(frame) {
    ...
    if (frame.profileId == "0000") {
      ...
      if (status != 0) {
        var errmsg =
          "binding failed, error code: %s" + sprintf("0x%02x", status);
  ...
  handle_ZDO_match_descriptor_request(frame) {
    ...
    if (
      devices[frame.remote64] &&
      devices[frame.remote64].endpoints &&
      devices[frame.remote64].endpoints.length
    ) {
  ...
  handle_HA_switchcluster_response(frame) {
    ...
    if (zcl_command == 0x0a) {
      ...
      if (attribute == 0x0000) {
        ...
        if (status != 0x00) {
          return logger.error(
            "Switch cluster read attribute response error status: " + status
          );
        }
  ...
  check_online_devices() {
    try {
      ...
      this.configured_devices.forEach((ieeeaddr) => {
  ...
  init() {
    ...
    devices.forEach((item) => {
      if (item.permission == iotdefinitions.PERMISSION_ALLOWED) {
        ...
        if (
          item.deviceid &&
          item.type == iotdefinitions.IOT_DEVICE_GATEWAY &&
          item.mcu == "xbee"
        ) {
    ...
    if (!this.gateway) {
      return logger.error(
        "XBEE chipset was configured for Zigbee. An XBEE gateway must exists in the database."
      );
    }
    ...
    xbee.on("frame_object", (frame) => {
      try {
        if (frame && frame.clusterId) {
          logger.debug(
            `frame type: ${frame.type} clusterid: ${frame.clusterId}`
          );
  ...
  dotasks() {
    ...
    if (currtime - this.last_onlinedevice_check > 30000) {
      // TODO: put this value back 30000) {
