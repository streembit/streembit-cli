...
function configure_reports(
  ...
  console.log(
    "configure reporting at " +
      address64 +
      " buffer: " +
      util.inspect(reportbuf)
  );
...
function network_address_request(address64) {
  ...
  console.log(
    "Network Address Request ClusterID=0x0000 buffer: " + util.inspect(narbuf)
  );
...
function read_ias_cie(frame) {
  ...
  //var enrolldata = [...enrollbuf];
  console.log(
    "IAS Zone enroll read request buffer: " + util.inspect(enrollbuf)
  );
...
function enroll_to_ias(frame) {
  setTimeout(() => {
    if (!is_enroll_sent) {
      ...
      //var enrolldata = [...enrollbuf];
      console.log(
        "IAS Zone enroll write request buffer: " + util.inspect(enrollbuf)
      );
      iaszone_enroll(frame.remote64, frame.remote16, 2, 1, enrollbuf);
  ...
  try {
    if (frame) {
      ...
      if (frame.clusterId == "8032") {
        ...
        console.log(
          "buffer length: %d, status: %d, devices length: %d, startindex: %d, count: %d",
          bufflen,
          status,
          devices_length,
          startindex,
        ...
        if (startindex + count < devices_length) {
          console.log(
            "continue to read routing table from index " + (startindex + count)
          );
        ...
        if (frame.profileId == "0000") {
          ...
          console.log(
            "match descriptor response data: " + util.inspect(respbuf)
          );
        ...
        if (frame.profileId == "0000") {
          ...
          console.log(
            "id: %s, status: %d, address16: %s",
            id.toString(16),
            status,
            destaddress.toString(16)
          );
          ...
          console.log(
            frame.remote64 +
              " devdata: " +
              util.inspect(devdata[frame.remote64])
          );
          ...
          //var sdrdata = [...sdrbuf];
          console.log(
            "Simple Descriptor Request data: " + util.inspect(sdrbuf)
          );
