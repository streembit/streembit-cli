...
streembit.bootclient = (function (client, logger, config, events) {
    ...
    function is_hostname(address) {
        var valid = address && typeof address === "string" && address.trim().length > 0;
        return valid;
    }
    ...
    client.discovery = function (address, callback) {
        if (is_ipaddress(address) || is_hostname(address)) {
            return callback(null, address);
        }
        else {
            callback("Invalid host address");
        }
    };
