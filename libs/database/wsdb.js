...
export class WsDb extends database {
  constructor() {
    super();
  }
  ...
  add_client(pkhash, publickey, token, isactive, account) {
    return new Promise((resolve, reject) => {
      try {
        const query =
          "INSERT INTO wsclients(pkhash, publickey, token, isactive, account, time_updated) VALUES (?,?,?,?,?,?)";
        this.database.run(
          query,
          [pkhash, publickey, token, isactive, account, +new Date()],
          (err) => {
            if (err) {
              return reject(err.message);
            }
  ...
  update_client(pkhash, token, isactive) {
    return new Promise((resolve, reject) => {
      try {
        ...
        let query =
          "UPDATE wsclients SET token = ?, isactive = ?, time_updated = ? WHERE pkhash = ?";
        this.database.run(
          query,
          [token, isactive, +new Date(), pkhash],
          (err) => {
            if (err) {
              return reject(err.message);
            }
  ...
  get_activeclients_count() {
    return new Promise((resolve, reject) => {
      let query =
        "SELECT COUNT(clientid) as total FROM wsclients WHERE isactive = 1;";
  ...
  async register(pkhash, publickey, token, account) {
    ...
    try {
      ...
      return Promise.reject(
        new Error(typeof err === "string" ? err : err.message)
      );
    }
  }
}
