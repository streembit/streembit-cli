...
class Node extends events.EventEmitter {
    ...
    connect(contact, callback) {
        let self = this;
        let seed = this._rpc._createContact(contact);
     ...
     * @param {Function} callback
     */
    _putValidatedKeyValue(item, callback) {
        let node = this;
     ...
     * @param {Function} callback
     */
    _ensureTransportState(callback) {
     ...
     * @private
     */
    _bindRPCMessageHandlers(options) {
        let self = this;
     ...
     * @private
     */
    _startReplicationInterval(replicateval) {
    ...
    _replicate() {
        let self = this;
        let stream = this._storage.createReadStream();
     ...
     * @private
     */
    _startExpirationInterval(expireval) {
     ...
     */
    _handlePing(incomingMsg) {
        let contact = this._rpc._createContact(incomingMsg.params.contact);
        let message = new Message({
            id: incomingMsg.id,
     ...
     * @param {Message} incomingMsg
     */
    _handleStore(incomingMsg) {
        let node = this;
        let params = incomingMsg.params;
        ...
        this._log.debug('received valid STORE from %s', params.contact.nodeID);
     ...
     * @param {Message} incomingMsg
     */
    _storeValidatedKeyValue(item, incomingMsg) {
        let node = this;
        let params = incomingMsg.params;

        this._storage.put(item.key, JSON.stringify(item), function (err) {
            let contact = node._rpc._createContact(incomingMsg.params.contact);
            let message = new Message({
                error: err,
     ...
     * @param {Message} incomingMsg
     */
    _handleFindNode(incomingMsg) {
        this._log.debug('received FIND_NODE from %j', incomingMsg.params.contact);

        let node = this;
        let params = incomingMsg.params;
        let contact = this._rpc._createContact(params.contact);

        let near = this._router.getNearestContacts(
            params.key,
            constants.K,
     ...
     * @param {Message} incomingMsg
     */
    _handleFindValue(incomingMsg) {
        let node = this;
        let params = incomingMsg.params;
        let contact = this._rpc._createContact(params.contact);
        let limit = constants.K;

        this._log.debug('received valid FIND_VALUE from %s', params.contact.nodeID);
        this._storage.get(params.key, function (err, value) {
            if (err || !value) {
                ...
                let notFoundMessage = new Message({
                    id: incomingMsg.id,
                    result: {
