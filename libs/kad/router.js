...
class Router extends events.EventEmitter {
     ...
     */
    lookup(type, key, callback, aggressiveLookup) {
        assert(['NODE', 'VALUE'].indexOf(type) !== -1, 'Invalid search type');

        let state = this._createLookupState(type, key, {
    ...
    removeContact(contact) {
        let index = utils.getBucketIndex(this._self.nodeID, contact.nodeID);
        ...
        try {
            ...
            this._log.error("set inactive_contacts error: %j", err);
        }
    ...
    getContactByNodeID(nodeID) {
        for (let k in this._buckets) {
            let contacts = this._buckets[k].getContactList();
     ...
     */
    _createLookupState(type, key, options) {
     ...
     */
    _iterativeFind(state, contacts, callback) {
        let self = this;
        let failures = 0;
     ...
     * @param {Function} callback
     */
    _queryContact(state, contactInfo, callback) {
        let self = this;
        let contact = this._rpc._createContact(contactInfo);
     ...
     * @param {Function} callback
     */
    _handleFindResult(state, msg, contact, callback) {
        let distance = utils.getDistance(state.hashedKey, contact.nodeID);
     ...
     * @param {Function} callback
     */
    _validateFindResult(state, msg, contact, done) {
        let self = this;
        let item = msg.result.item;
     ...
     */
    _addToShortList(state, contacts) {
     ...
     */
    _removeFromShortList(state, nodeID) {
     ...
     * @param {Function} callback
     */
    _handleQueryResults(state, callback) {
        if (state.foundValue) {
        ...
        let shortlistFull = state.shortlist.length >= constants.K;
        let shouldNotContinue = state.aggressiveLookup ?
        ...
        let remainingContacts = _.reject(state.shortlist, function (c) {
        ...
        let remcontacts = remainingContacts.splice(0, constants.ALPHA);
        //this._log.debug('contacts to _iterativeFind: %j', remcontacts);

        let contacts = [];
        for (let i = 0; i < remcontacts.length; i++) {
            let nodeID = remcontacts[i].nodeID;
     ...
     * @param {Function} callback
     */
    _handleValueReturned(state, callback) {
        let self = this;

        let distances = state.contactsWithoutValue.map(function (contact) {
            return {
     ...
     */
    refreshBucketsBeyondClosest(contacts, done) {
        let bucketIndexes = Object.keys(this._buckets);
        let leastBucket = _.min(bucketIndexes);

        function bucketFilter(index) {
            return index > leastBucket;
        }

        let refreshBuckets = bucketIndexes.filter(bucketFilter);
        let queue = async.queue(this.refreshBucket.bind(this), 1);
     ...
     * @param {Function} callback
     */
    refreshBucket(index, callback) {
        let random = utils.getRandomInBucketRangeBuffer(index);
     ...
     */
    findNode(nodeID, options, callback) {
        let self = this;
    ...
    updateContact(contact, callback) {
        let bucketIndex = utils.getBucketIndex(this._self.nodeID, contact.nodeID);
        ...
        try {
            ...
            this._log.error("remove inactive_contacts error: %j", err);
        }
    ...
    * @param {Function} callback
    */
    _moveContactToTail(contact, bucket, callback) {
     ...
     * @param {Function} callback
     */
    _moveContactToHead(contact, bucket, callback) {
     ...
     * @param {Function} callback
     */
    _pingContactAtHead(contact, bucket, callback) {
        let self = this;
        let ping = new Message({ method: 'PING', params: { contact: this._self } });
        let head = bucket.getContact(0);
    ...
    getNearestContacts(key, limit, nodeID) {
        let self = this;
        let contacts = [];
        let index = utils.getBucketIndex(this._self.nodeID, utils.createID(key));
        ...
        function addNearestFromBucket(bucket) {
            self._getNearestFromBucket(
                ...
                let isContact = contact instanceof Contact;
                let poolNotFull = contacts.length < limit;
                let notRequester = contact.nodeID !== nodeID;
     ...
     * @returns {Array}
     */
    _getNearestFromBucket(bucket, key, limit) {
        if (!bucket) {
            return [];
        }

        let nearest = bucket.getContactList().map(function addDistance(contact) {
            return {
                contact: contact,
