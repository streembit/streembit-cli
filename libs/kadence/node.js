...
export class Node {
  ...
  eventHandlers() {
    events.on("kad_message", (message, req, res) => {
      req.on("error", (err) => {
  ...
  validateMessageIdentity(msg) {
    try {
      ...
      const verifyId = utils
        .hash160(Buffer.from(`${hostname}:${port}`))
        .toString("hex");
  ...
  join(callback) {
    if (!this.seeds || this.seeds.length < 1) {
      this.logger.warn(
        "There are no Kademlia seeds defined, the node is not connected to any seeds"
      );
    ...
    async.mapSeries(
      ...
      (item, cbfunc) => {
        ...
        const result = { seed: seed, error: null };
        try {
  ...
  async init(options, callback) {
    if (
      ...
      return callback(
        "A logger that implements the error, info, warn and debug methods must be passed to the Kademlia node"
      );
    }

    const nodeIdentity = Buffer.from(
      `${options.contact.hostname}:${options.contact.port}`
    );
    ...
    this.seeds = [
      ...options.seeds,
      ...peers.filter((p) => options.seeds.every((s) => p.id !== s.id)),
    ];
    ...
    this.node.listen(options.contact.port, options.contact.hostname, (err) => {
      ...
      this.logger.info(
        `Identity ${options.identity} is listening on port ${options.contact.port}`
      );

      // join the Kademlia network
      this.join((count) => {
        const routcount = this.node.router.size;
        this.peercount = routcount > count ? routcount : count;
        if (this.peercount > 0) {
          this.logger.info(
            `Kademlia node connected to ${this.peercount} peers`
          );
        } else {
