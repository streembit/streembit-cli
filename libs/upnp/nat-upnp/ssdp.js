...
export class Ssdp extends EventEmitter {
  ...
  createSockets() {
    ...
    this.sockets = Object.keys(interfaces).reduce((a, key) => {
      return a.concat(
        interfaces[key]
          .filter((item) => {
  ...
  search(device, promise) {
    ...
    const ondevice = (info, device_address, socket_addess) => {
      if (promise._ended) return;
  ...
  createSocket(item) {
    ...
    process.nextTick(() => {
      ...
      const onready = () => {
        if (self._boundCount < self.sockets.length) {
          return;
        }

        self._bound = true;
        self._queue.forEach((item) => {
      ...
      socket.address = item.address;
      socket.bind(self._sourcePort, item.address);
    });
